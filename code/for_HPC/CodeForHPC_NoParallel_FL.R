...
# Load model data
load("/home/nhaydt/NRT_Project/fung_lit_OUT_IC.Rdata")
load("/home/nhaydt/NRT_Project/bact_lit_OUT_IC.Rdata")
...
grid <- read.csv("/home/nhaydt/NRT_Project/data/FL_grid_fourth_random.csv", stringsAsFactors=FALSE,
                 header = TRUE) ## Across TX
...
# BACTERIA MODEL
start_time <- Sys.time()
predY_blit <-  predict(full_blit_mod, 
                       Gradient=Gradient_blit,
                       expected = TRUE,
                       nParallel = 1,
                       predictEtaMean = TRUE)
end_time <- Sys.time()
...
# FUNGI MODEL
start_time <- Sys.time()
predY_flit <-  predict(full_flit_mod, 
                       Gradient=Gradient_flit,
                       expected = TRUE,
                       nParallel = 1,
                       predictEtaMean = TRUE)
end_time <- Sys.time()
...
write.csv(pred.df.blit, file = "/home/nhaydt/NRT_Project/mean_bact_preds_fourth_FL.csv")

EpredY <- Reduce("+",predY_flit )/length(predY_flit )
pred.df.flit <- as.data.frame(EpredY)
write.csv(pred.df.flit, file = "/home/nhaydt/NRT_Project/mean_fung_preds_fourth_FL.csv")