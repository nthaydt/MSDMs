...
# grid <- read.csv("/home/nhaydt/NRT_Project/data/AR_grid_cellnum_random.csv", stringsAsFactors=TRUE). ## Across AR
grid <- read.csv("/home/nhaydt/NRT_Project/data/US_grid_500thous_random.csv", stringsAsFactors=FALSE,
                 header = TRUE) ## Across the US
...
write.csv(pred.df.blit, file = "/home/nhaydt/NRT_Project/mean_bact_preds_1mill_US_20Ap.csv")

EpredY <- Reduce("+",predY_flit )/length(predY_flit )
pred.df.flit <- as.data.frame(EpredY)
write.csv(pred.df.flit, file = "/home/nhaydt/NRT_Project/mean_fung_preds_1mill_US_20Ap.csv")
